...
const AppointmentDetailModal = ({ appointment, isOpen, onClose }) => {
  ...
  const cancelMutation = useMutation({
    mutationFn: () =>
      appointmentsAPI.cancel(appointment._id, "Cancelled by provider"),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["calendarAppointments"] });
      queryClient.invalidateQueries({ queryKey: ["appointments"] });
      onClose();
          ...
          {!isCancelled && (
            <div className="space-y-4 border-t border-border pt-4">
              ...
              <div>
                ...
                <textarea
                  className="w-full rounded-xl border border-input bg-background px-4 py-3 text-foreground text-sm placeholder:text-muted-foreground focus:outline-hidden focus:ring-2 focus:ring-ring"
                  rows="3"
